(function (global) {
  ...
  function stateAllowed(contract, value) {
    return contract.state.allowed.some(item => sameState(item, value));
  }

  function part(root, name) {
    return root.querySelector(`[data-part="${CSS.escape(name)}"]`);
  }
  ...
  function normalizeStyleValue(prop, value) {
    ...
    if (prop === "opacity") return String(value);
    return String(value);
  }
  ...
  function applyPartStyle(element, values) {
    ...
    for (const [prop, raw] of Object.entries(values)) {
      ...
      if (prop === "transformOrigin") {
        element.style.transformOrigin = String(raw);
      } else if (ANIMATABLE_PROPS.includes(prop)) {
        element.style[prop] = normalizeStyleValue(prop, raw);
      }
    }
  }
  ...
  function findTransition(contract, from, to) {
    return (contract.transitions || []).find(item => sameState(item.from, from) && sameState(item.to, to)) || null;
  }
  ...
  function computedFirstFrame(element, templateFrame) {
    ...
    for (const prop of ANIMATABLE_PROPS) {
      if (Object.prototype.hasOwnProperty.call(templateFrame, prop)) frame[prop] = computed[prop];
    }
    return frame;
  }
  ...
  function create(root, contract) {
    ...
    const missingParts = [...new Set([...(contract.geometry.stable_parts || []), ...(contract.geometry.actors || [])])]
      .filter(name => !part(root, name));
    ...
    function cancelActive({ preserveVisual = true } = {}) {
      const snapshots = preserveVisual
        ? active.map(item => ({ element: item.element, style: styleSnapshot(item.element) }))
        : [];
      ...
      if (preserveVisual) {
        for (const { element, style } of snapshots) {
          for (const prop of ANIMATABLE_PROPS) {
            if (style[prop] !== undefined) element.style[prop] = style[prop];
          }
        }
      }
    }
    ...
    function finishTarget(token, value) {
      ...
      if (!sameState(value, latestRequested) && contract.behavior.interrupt_policy !== "complete") return;
      applyVisualState(root, contract, value);
      current = value;
      target = value;
      active = [];
      if (queuedTarget !== null && !sameState(queuedTarget, current)) {
    ...
    function beginTransition(nextTarget) {
      ...
      if (reducedMotion || contract.accessibility.reduced_motion === "no-transient-motion") {
        return setState(nextTarget);
      }
      if (sameState(nextTarget, current) && active.length === 0) return getState();

      const policy = contract.behavior.interrupt_policy;
      if (active.length && policy === "complete") {
        queuedTarget = nextTarget;
        return getState();
      }
      ...
      if (!transition && wasActive && (policy === "retarget" || policy === "reverse")) {
        ...
        transition = {
          from: fromState,
          to: nextTarget,
          duration_ms: contract.motion.default_duration_ms,
          easing: contract.motion.default_easing,
          tracks: candidateParts.map(name => {
            const end = { offset: 1 };
            for (const prop of ANIMATABLE_PROPS) {
              if (Object.prototype.hasOwnProperty.call(targetParts[name], prop)) end[prop] = targetParts[name][prop];
            }
            return { part: name, keyframes: [{ ...end, offset: 0 }, end] };
          })
        };
      }
      if (!transition) {
        lastError = { code: "TRANSITION_MISSING", from: fromState, to: nextTarget };
        return setState(nextTarget);
      }
      ...
      for (const track of transition.tracks || []) {
        ...
        const animation = element.animate(frames, {
          duration,
          delay,
          easing: track.easing ?? defaultEasing,
          fill: "both",
          iterations: 1
        });
    ...
    function seek(progress) {
      ...
      if (reducedMotion && p > 0) {
        setState(latestRequested);
        return getState();
      }
      if (p < 1) {
        for (const item of active) {
          item.animation.pause();
          item.animation.currentTime = item.delay + item.duration * p;
        }
        return getState();
      }

      let guard = 0;
      while (active.length && guard < 8) {
        for (const item of active) {
          item.animation.pause();
          item.animation.currentTime = item.delay + item.duration;
        }
        const finalTarget = target;
        const token = transitionToken;
        finishTarget(token, finalTarget);
        guard += 1;
      }
      if (active.length) lastError = { code: "SETTLE_GUARD_EXCEEDED", latestRequested };
      return getState();
    }
    ...
    function setReducedMotion(enabled) {
      ...
      if (reducedMotion && active.length) setState(latestRequested);
      return getState();
    }
    ...
    function reset() {
      reducedMotion = false;
      return setState(contract.state.initial);
    }

    function getTestScenarios() {
      return (contract.verification.scenarios || []).map(item => ({
        name: item.name,
        start: item.start,
        firstTarget: item.first_target,
        interruptAt: item.interrupt_at,
        secondTarget: item.second_target,
